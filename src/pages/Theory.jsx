  ...
  return (
    <div>
      ...
      <div className="theory-chap-scroll">
        {chapters.map(c=>(
          <button key={c.id} onClick={()=>pick(c)} className="theory-chap-btn" style={{
            padding:'.38rem .75rem',borderRadius:8,flexShrink:0,
            background:ch.id===c.id?`${c.color}18`:'var(--sur)',
            border:`1px solid ${ch.id===c.id?c.color+'40':'var(--brd)'}`,
            color:ch.id===c.id?c.color:'var(--txt3)',
            cursor:'pointer',fontSize:'.78rem',fontWeight:ch.id===c.id?600:400,
            fontFamily:'var(--fd)',transition:'all .13s',whiteSpace:'nowrap',
          }}>{c.n}. {c.title}</button>
        ))}
      </div>
      ...
      <div className="theory-grid" style={{display:'grid',gridTemplateColumns:'210px 1fr',gap:'1.3rem',alignItems:'start'}}>
        ...
        {/* Desktop sidebar */}
        <div style={{position:'sticky',top:'1rem'}} className="theory-sidebar-desktop">
          <div style={{fontSize:'.65rem',color:'var(--txt3)',fontFamily:'var(--fc)',marginBottom:'.45rem',textTransform:'uppercase',letterSpacing:'.06em'}}>
            Chương học
          </div>
          {chapters.map(c=>(
          ...
          <div className="card" style={{padding:'1.1rem',marginBottom:'.85rem',background:`${ch.color}07`,borderColor:`${ch.color}30`}}>
            <div style={{display:'flex',alignItems:'center',gap:'.65rem',marginBottom:'.35rem',flexWrap:'wrap'}}>
              ...
              <div>
                ...
                <h2 style={{fontFamily:'var(--fm)',fontSize:'1.08rem',color:'var(--txt)'}}>{ch.title}</h2>
              </div>
            </div>
            <div style={{display:'flex',gap:'.35rem',flexWrap:'wrap',alignItems:'center'}}>
              <Diff level={ch.diff}/>
              <span style={{fontSize:'.72rem',color:'var(--txt3)'}}>⏱ {ch.time}</span>
              <span style={{fontSize:'.72rem',color:'var(--txt3)'}}>· {ch.sections.length} phần</span>
              <span style={{fontSize:'.72rem',color:'var(--txt3)'}}>· {ch.quiz.length} quiz</span>
              <span style={{fontSize:'.72rem',color:'var(--txt3)'}}>· {ch.exercises.length} bài tập</span>
            </div>
          </div>
