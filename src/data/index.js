...
// Thay thế file data hiện tại — giữ nguyên tên export `projects`
...
export const projects = [
  // ── CƠ BẢN (p1–p5) ──────────────────────────────────────────────────────
  {
    ...
    deliver: ['topology.py', 'flow_analysis.txt', 'diagram.png'],
    steps: [
      'Cài đặt Mininet, kiểm tra phiên bản: mn --version',
      'Chạy lệnh mn --topo linear,3,2 để tạo topology 3 switch + 6 host',
      'Kiểm tra kết nối cơ bản bằng pingall và ghi nhận kết quả',
      'Dùng lệnh ovs-ofctl dump-flows s1/s2/s3 để dump flow table',
      'Phân tích flow table: action, priority, match fields',
      'Vẽ sơ đồ topology bằng tay hoặc export từ Mininet',
      'Viết flow_analysis.txt mô tả từng flow entry chi tiết',
    ],
    rec: false,
    recReason: null,
  },
  {
    ...
    steps: [
      'Khởi tạo simple topology: 1 switch, 2 host (h1, h2)',
      'Cài Apache2 trong namespace h1: h1 apt-get install apache2',
      'Khởi động server: h1 service apache2 start',
      'Từ h2 chạy wget hoặc curl để kiểm tra HTTP response thành công',
      'Đo throughput: h2 ab -n 1000 -c 10 http://10.0.0.1/',
      'Song song đo bandwidth bằng iperf3 giữa h1 và h2',
      'Export số liệu ra benchmark.csv, so sánh hai phương pháp',
      'Viết script.sh tự động hóa toàn bộ quy trình trên',
    ],
    rec: false,
    recReason: null,
  },
  {
    ...
    steps: [
      'Tạo topology đơn giản: 1 switch, 3 host',
  ...
  },
  {
    ...
    steps: [
      ...
      'Khởi tạo net = Mininet(topo, controller=RemoteController)',
  ...
  // ── TRUNG BÌNH (p6–p13) ──────────────────────────────────────────────────
  {
    ...
    steps: [
      'Cài đặt Ryu: pip install ryu, kiểm tra ryu-manager --version',
      'Tạo file l2_switch.py, import RyuApp, OFPPacketIn, OFPFlowMod',
      'Implement mac_to_port = {} làm bảng MAC learning',
      'Xử lý PACKET_IN: học MAC nguồn, tra cứu MAC đích',
      'Nếu biết port đích: cài flow rule unicast + gửi packet-out',
      'Nếu chưa biết: flood ra tất cả port (OFPP_FLOOD)',
      'Khởi Ryu: ryu-manager l2_switch.py, kết nối Mininet remote controller',
      'Dùng tcpdump so sánh traffic: hub (flood) vs learning switch (unicast)',
      'Viết test_script.py kiểm tra bảng MAC, đo số lần flood giảm dần',
      'Tổng hợp report.md: giải thích thuật toán, kết quả đo lường',
    ],
    rec: true,
    recReason: 'Nền tảng bắt buộc của SDN. Hiểu được L2 Learning Switch nghĩa là nắm vững OpenFlow pipeline, Packet-In/Out, Flow-Mod — 3 khái niệm cốt lõi mà mọi đề tài nâng cao đều dùng lại.',
  },
  {
  ...
  },
  {
    ...
    steps: [
      ...
      'Viết firewall.py: load rules từ JSON config file',
      'Xử lý PACKET_IN: parse IP header, so khớp với rule table',
      'Match ALLOW: install flow với priority cao; không match: install DROP flow',
      'Logging: mỗi violation ghi vào violations.log (IP src, dst, timestamp)',
      'Tạo test topology: 4 host, 2 subnet, 2 switch',
      'Viết test_cases.py: kiểm tra kịch bản allow/deny tự động',
      'Thêm tính năng dynamic rule update qua REST endpoint',
      'Tổng hợp security_report.md: kiến trúc, test results, phân tích',
    ],
    rec: true,
    recReason: 'Bảo mật mạng có tính ứng dụng thực tế cao. Firewall SDN logic tập trung nên dễ trình bày hơn iptables truyền thống. Có logging giúp tạo dữ liệu phân tích cho chương đánh giá của luận văn.',
  },
  {
    ...
    steps: [
      'Tạo topology: 1 OVS switch, 8 host, phân nhóm VLAN 10/20/30',
      'Cấu hình access port: ovs-vsctl set port hN-eth0 tag=VLAN_ID',
      'Cấu hình trunk port giữa 2 switch: cho phép tất cả VLAN',
      'Cài flow rule: chỉ forward trong cùng VLAN ID, drop khác VLAN',
  ...
  },
  {
    ...
    steps: [
      ...
      'Demo: tự động add/remove rules dựa theo giờ (giờ cao điểm)',
      'Viết api_docs.md dạng OpenAPI: endpoint, request/response schema',
    ],
    rec: true,
    recReason: 'Kết hợp được 3 kỹ năng thực tế: SDN + web API + automation. Demo trực quan, ấn tượng trong buổi bảo vệ. Có thể mở rộng thành dashboard hoặc tích hợp với đề tài monitoring.',
  },
  {
    ...
    steps: [
      'Cài sFlow-RT hoặc dùng ovs-vsctl set-sflow để bật sFlow trên OVS',
      'Viết collector.py: nhận UDP sFlow packets, parse counters',
      'Lưu metrics vào SQLite hoặc in-memory time series',
      'Tạo Flask app với endpoint /api/metrics trả JSON',
      'Dùng Server-Sent Events (SSE) để push data real-time',
      'Frontend: Chart.js line chart bandwidth, bar chart packet rate',
      'Thêm trang topology view: hiển thị switch/host dạng node-link',
      'Auto-refresh mỗi 5 giây, hiển thị cảnh báo khi vượt ngưỡng',
    ],
    rec: false,
    recReason: null,
  },
  {
    ...
    steps: [
      'Tạo topology: 1 nguồn, 8 receiver, 4 switch dạng cây',
      'Cài OpenFlow Group loại ALL cho flooding baseline',
      'Implement Spanning Tree tính multicast tree tối ưu',
      'Cài Group loại ALL chỉ với port trên tree (loại bỏ redundant path)',
      'Đo bandwidth tiêu thụ: flooding vs tree-based với iperf3 multicast',
      'Đếm số packet duplicate ở receiver trong mỗi kịch bản',
      'Đo CPU usage của switch khi dùng 2 phương pháp',
      'Viết comparison.md: bảng số liệu, phân tích trade-off',
    ],
    rec: false,
    recReason: null,
  },
  ...
  // ── NÂNG CAO (p14–p20) ──────────────────────────────────────────────────
  {
    ...
    steps: [
      'Nghiên cứu cấu trúc Fat Tree k=4: 20 switch, 16 host, 3 tier',
      'Viết fat_tree.py theo công thức: k pods, k/2 edge, k/2 aggr, (k/2)² core',
      'Đặt địa chỉ IP theo scheme chuẩn: 10.pod.switch.host',
      'Implement ECMP trong controller: tìm nhiều equal-cost paths',
      'Hash 5-tuple (src IP, dst IP, src/dst port, proto) để chọn path',
      'Benchmark bisection bandwidth: iperf3 all-to-all 2 nhóm 8 host',
      'So sánh với single-path: đo throughput tổng, Jain fairness index',
      'Vẽ CDF throughput mỗi flow trong 2 kịch bản',
      'Viết benchmark.md: phân tích, so sánh với paper Al-Fares 2008',
    ],
    rec: true,
    recReason: 'Fat Tree là topology chuẩn của data center hiện đại (Google, Facebook). Có nền tảng lý thuyết từ paper SIGCOMM 2008, dễ so sánh số liệu với nghiên cứu gốc. Phù hợp làm đề tài nghiên cứu xuất sắc.',
  },
  {
    ...
    steps: [
      'Topology: nhiều attacker host + victim + Ryu controller',
      'Sinh traffic DDoS: h_atk hping3 -S --flood -V -p 80 victim',
      'Implement polling: mỗi 5s lấy flow stats qua Ryu REST API',
      'Tính entropy của IP nguồn trong cửa sổ thời gian trượt',
      'Entropy thấp đột ngột = traffic tập trung = dấu hiệu DDoS',
      'Khi phát hiện: lấy top-N IP nguồn có lưu lượng bất thường cao',
      'Gửi Flow-Mod drop với match IP src của attacker, priority cao nhất',
      'Sinh alert log: timestamp, attacker IP, traffic volume',
      'Đo precision/recall: đúng/sai/bỏ sót trong nhiều kịch bản',
      'Viết research_report.pdf theo chuẩn IEEE: Abstract, Related Work, Method, Evaluation',
    ],
    rec: true,
    recReason: 'Chủ đề thời sự, kết hợp bảo mật với phân tích thống kê (entropy). Cấu trúc report theo IEEE giúp chuẩn bị kỹ năng viết báo cáo khoa học, rất phù hợp nếu muốn làm luận văn tốt nghiệp.',
  },
  {
    ...
    steps: [
      'Thiết kế môi trường Gym: state (link utilization), action (chọn path), reward (-latency)',
      'Viết mininet_env.py implement gym.Env: reset(), step(), observation_space',
      'Tạo DQN network PyTorch: input=network state vector, output=Q-values',
      'Implement experience replay buffer và target network',
      'Training loop: agent tương tác môi trường, cập nhật Q-network',
      'Sinh traffic ngẫu nhiên trong training (thay đổi load liên tục)',
      'Mỗi episode log: avg latency, packet loss, reward',
      'So sánh DQN với shortest-path routing và random routing',
      'Vẽ learning curve: cumulative reward theo số episode',
    ],
    rec: false,
    recReason: null,
  },
  {
    ...
    steps: [
      'Định nghĩa ngữ pháp intent: VERB + TRAFFIC_TYPE + FROM + IP + TO + IP',
      ...
      'Test 30+ câu lệnh tiếng Anh đa dạng, đo accuracy',
      'Xử lý trường hợp mơ hồ: hỏi xác nhận trước khi apply',
  ...
  },
  {
    ...
    steps: [
      ...
      'Cài inter-VLAN routing với subinterface hoặc L3 switch',
      ...
      'Viết Flask monitoring: real-time traffic, VLAN stats, alert system',
      'Integrate ONOS controller thay Ryu cho production-grade control',
      'Load test: sinh traffic 80% capacity, kiểm tra SLA từng dịch vụ',
      'Viết tài liệu kỹ thuật chuẩn RFC: requirements, design, implementation, test',
  ...
  },
  {
    ...
    steps: [
      'Thiết kế kiến trúc: physical net → Kafka → digital twin (Mininet)',
      'Viết collector agent: thu thập link state, flow table, port stats liên tục',
      'Publish metrics vào Kafka topic với schema Avro',
      'Viết shadow_net.py: subscribe Kafka, sync trạng thái vào Mininet mirror',
      'Implement state reconciliation: phát hiện drift giữa real và twin',
      'API thử nghiệm thay đổi trên twin trước khi apply vào real net',
      'Đo độ chính xác: so sánh latency/throughput real vs twin',
      'Simulate failure trên twin, kiểm tra recovery plan trước khi triển khai',
    ],
    rec: false,
    recReason: null,
  },
  {
    ...
    steps: [
      ...
      'Nghiên cứu P4 pipeline: parser → match-action → deparser',
      'Viết basic.p4: định nghĩa header Ethernet, IPv4, custom header',
      'Implement parser: extract headers từ packet bytes',
      'Viết match-action table: IPv4 forward dựa theo dst IP',
      'Compile: p4c --target bmv2 --arch v1model basic.p4',
      'Khởi chạy BMv2 trong Mininet thay OVS switch',
      'Viết control_plane.py dùng P4Runtime API để populate table entries',
      'Test toàn diện: ping, iperf3, kiểm tra custom header xử lý đúng',
      'Mở rộng: thêm telemetry header (INT) đo in-band network telemetry',
    ],
    rec: false,
    recReason: null,
  },
